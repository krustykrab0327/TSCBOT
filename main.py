...
def initialize_system():
    ...
    try:
        # 先授權
        gc = pygsheets.authorize(service_account_file="/secrets/service_account.json")
        # 先開啟試算表，這行執行完全域變數 sheet 才有值
        sheet_url = os.environ.get("GOOGLESHEET_URL")
        sheet = gc.open_by_url(sheet_url)
        
        # --- Firestore  ---
        # 直接使用 Client.from_service_account_json() 讀取檔案
        db = firestore.Client.from_service_account_json("/secrets/service_account.json")
...
def retrieve_top_n(query, n=2, threshold=3, high_threshold=10):
    """取得最相似的問題
    ##作法
